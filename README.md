...
**Docker volume binds from `mc-guard`:** `backup.sh` / `restore.sh` use **`docker run -v …`**. The daemon resolves bind sources on the **host**, not inside the container. `BACKUP_DIR` stays **`/backups`** for `find`, `wc`, and Telegram paths, while **`BACKUP_DOCKER_HOST_DIR`** is set by **`mc-guard`** to **`CREEPWATCH_PROJECT_DIR/backups`** on the host (or **`CREEPWATCH_BACKUP_DIR_HOST`** if you override). Without **`CREEPWATCH_PROJECT_DIR`**, `/backup` from Telegram cannot infer the host backups path.

**Telegram:** Admins use **`/backup`** (alias **`/bu`**) and **`/restore`** (`/rs`): **`/restore slots`** shows slots **1–3** (newest first) with UTC times parsed from filenames; **`/restore list`** lists many names; **`/restore last`** or **`/restore 1`**–**`3`** or **`/restore <filename>`** runs a restore. While **`mc-guard`** runs restore, it sets **`RESTORE_PROGRESS_CHAT_ID`** so **`restore.sh`** can **`sendMessage`** short progress lines (start, stop container, extract, start container, done), like backup. If **anyone is online**, the bot **queues** the job, sends a **`tellraw @a`** heads-up in Minecraft, and runs the script automatically once **`rcon list`** shows zero players (polled about every 45 seconds). Only **one** queued maintenance job is allowed at a time. **`/update`** is blocked while a backup or restore is queued.

When you run **`/backup`** with an empty server, **`backup.sh`** sends **short progress lines** to your private Telegram chat (preflight → snapshot → gzip OK → R2 if configured → finished), using the same bot token and **`BACKUP_PROGRESS_CHAT_ID`** injected by mc-guard. **`mc-guard`** also calls **`sendChatAction`** with **`upload_document`** on that chat every few seconds while the subprocess runs; Telegram shows that **above the message field** (not as a chat bubble), often as “sending file…” or similar.
