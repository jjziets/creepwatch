...
- **Telegram backups / restore** — `/backup` and `/restore` (`list`, `slots`, `last`, `1`–`3` for Nth-newest, or a specific `minecraft-*.tar.gz`); if anyone is online the op is **scheduled** and players see an in-game notice; otherwise it runs immediately. Restore sends **stage lines** to the admin chat (same pattern as backup progress).
...
| `/restore` | `/rs` | `slots` · `list` · `last` · `1`–`3` · `<minecraft-…Z.tar.gz>` — restore world from backup (scheduled if online) |
...
**Docker volume binds from `mc-guard`:** `backup.sh` / `restore.sh` use **`docker run -v …`**. The daemon resolves bind sources on the **host**, not inside the container. `BACKUP_DIR` stays **`/backups`** for `find`, `wc`, and Telegram paths, while **`BACKUP_DOCKER_HOST_DIR`** is resolved by **`mc-guard`** in this order: **`CREEPWATCH_BACKUP_DIR_HOST`** (explicit override) → **`CREEPWATCH_PROJECT_DIR/backups`** → `docker inspect mc-guard` self-discovery of the **`/backups`** bind mount source. The third path means a fresh deploy does not need either env var set — the operator only configures the bind mount in `docker-compose.yml`.

**Telegram progress board:** Admins use **`/backup`** (alias **`/bu`**) and **`/restore`** (`/rs`). Instead of one message per stage, **`mc-guard`** sends a single message and **edits** it as the script reports each step. Both scripts write structured `PROGRESS\t<step>\t<status>\t<label>\t<detail>` lines to a per-run file (`BACKUP_PROGRESS_FILE` / `RESTORE_PROGRESS_FILE`) that `mc-guard` tails by offset every ~0.6s and re-renders, throttled to one edit per ~1.2s. The board shows all steps with status icons (⏳ pending · 🔄 running · ✅ done · ⏭️ skipped · ❌ failed) and a progress bar; the header switches to `✅ done in Xm` or `❌ failed at step N` on exit. **`/backup`** has 8 steps (preflight, save-all flush, save-off, compress, save-on, verify archive, R2 upload, local retention); **`/restore`** has 4 (safety snapshot, stop, wipe+extract, start). `mc-guard` also calls **`sendChatAction`** with **`upload_document`** every few seconds so Telegram shows a hint above the input.

**Restore safety snapshot:** **Step 1 of every `/restore` is a fresh `pre-restore-<UTC>.tar.gz` snapshot of the current world**, taken before anything destructive happens. The restore aborts if that snapshot fails or is too small — `restore.sh` will not wipe the live volume without something on disk to fall back to. Pre-restore archives live alongside regular `minecraft-*.tar.gz` archives in **`BACKUP_DIR`** but use a different prefix so **`BACKUP_MAX_ARCHIVES`** does not prune them; the newest **`RESTORE_PRERESTORE_KEEP`** (default 3) are retained.

**24h-gap slot rule:** **`/restore slots`** (and `/restore 1`–`3`/`last`) walks the archives newest-first and picks the first archive plus each successive archive that is at least **24 hours older** than the previous one. Same-day repeat `/backup` runs only refresh slot 1 — slots 2 and 3 stay anchored to day-1 / day-2 snapshots so an overnight rollback target is always available. The picker lives in `mc_guard.pick_slot_archives` and is mirrored in shell (`slot_archives_24h_gap` in `scripts/restore.sh` / `scripts/backup.sh`).

**Retention (no fill-up):** by default the local backup directory **mirrors what is on R2**, with R2 itself trimmed to the slot set above (max **`R2_RETAIN_COUNT`**, default 3). When R2 is not configured the same slot rule applies locally with **`BACKUP_SLOT_COUNT`** (default 3). Set **`BACKUP_MAX_ARCHIVES`** to force a flat newest-N retention if the 24h-gap behaviour does not fit; that disables slot anchoring.

**Local-vs-R2 verification on restore:** `backup.sh` records the local archive's SHA-256 as **R2 user metadata `sha256`**. On `/restore`, `restore.sh` HEADs the requested object on R2; if the local file's SHA-256 matches the recorded value it skips re-downloading and restores from local. If local is missing or the hash differs, it pulls from R2 and re-verifies. Older R2 objects without `sha256` metadata fall back to "download and trust gzip integrity check".

**R2 mirror indicator in `/restore` listings:** Both **`/restore slots`** and **`/restore list`** annotate each archive with **✅** (also on R2) or **📍** (local only). When R2 is not configured or the bucket is unreachable, the indicator is hidden and a footer notes "R2 mirror status: not configured or bucket unreachable" so an outage isn't misread as "everything is local-only". Mc-guard caches the R2 listing for 30s and invalidates the cache automatically after every successful `/backup`.

**Persistent logs and `/logs`:** Both scripts append to **`/data/logs/backup.log`** and **`/data/logs/restore.log`** (`BACKUP_LOG_DIR` / `RESTORE_LOG_DIR` to override, falls back to `/tmp/creepwatch-logs` if `/data` is not writable). Each line is prefixed with the UTC timestamp and the run ID so multiple runs interleave readably. Use **`/logs backup [N]`** or **`/logs restore [N]`** from Telegram to receive the last `N ≤ 50` lines in a code block — handy for post-mortems on failed runs without SSH.

If **anyone is online**, the bot **queues** the job, sends a **`tellraw @a`** heads-up in Minecraft, and runs the script automatically once **`rcon list`** shows zero players (polled about every 45 seconds). Only **one** queued maintenance job is allowed at a time. **`/update`** is blocked while a backup or restore is queued.
...
**Host / cron:** You can still run **`scripts/backup.sh`** from the host (set **`BACKUP_DIR`**, optional **`DOCKER_REAL`**, **`CREEPWATCH_ENV_FILE`** for Telegram failure alerts). When invoked outside `mc-guard`, neither **`BACKUP_PROGRESS_FILE`** nor **`RESTORE_PROGRESS_FILE`** is set, so the scripts skip the structured progress events but still write the persistent log. **`scripts/restore.sh`** needs **`CREEPWATCH_PROJECT_DIR`** pointing at the compose project and **`BACKUP_DIR`**; step 1 always takes a `pre-restore-<UTC>.tar.gz` safety snapshot, then it **`stop`s** `minecraft`, wipes the **`minecraft_data`** volume contents from a sidecar container, extracts the chosen tarball, then **`start`s** `minecraft` again. Run **`restore.sh slots`** on the host to print slots **1–3** (no compose needed for that subcommand). Arguments: **`slots`** | **`last`** | **`1`**–**`3`** | **`minecraft-…tar.gz`**.
...
5. **Postflight**: archive must be at least **`BACKUP_MIN_ARCHIVE_BYTES`** (default **1 MiB**); then a quick **`gzip -l`** header read via **`BACKUP_GZIP_TOOLS_IMAGE`** (default **`debian:bookworm-slim`** — GNU gzip; Alpine’s BusyBox gzip has no **`-l`**). For tiny worlds you can set **`BACKUP_STRICT_GZIP_TEST=1`** to run full **`gzip -t`** (reads the whole archive — can be slow or memory-heavy).
